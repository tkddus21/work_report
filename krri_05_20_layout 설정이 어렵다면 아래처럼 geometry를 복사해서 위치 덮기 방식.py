...
from PyQt5.QtWidgets import QGraphicsScene, QGraphicsPixmapItem, QGraphicsView, QVBoxLayout
...
class TrackBotApp(QMainWindow):
    def __init__(self):
        ...
        # 1. 새로운 뷰 생성
        self.line_camera_view = BoxDrawingView(self)
        self.line_camera_view.setObjectName("LineCamera")
        self.line_camera_view.setDragMode(QGraphicsView.ScrollHandDrag)
        self.line_camera_view.setRenderHint(QPainter.Antialiasing)

        # 2. 기존 layout 가져오기
        layout = self.ui.LineCamera.parentWidget().layout()

        # 3. 기존 위젯 바꾸기
        if layout is not None:
            layout.replaceWidget(self.ui.LineCamera, self.line_camera_view)
            self.ui.LineCamera.deleteLater()
            self.ui.LineCamera = self.line_camera_view
        else:
            print("❌ [Error] LineCamera 부모에 layout이 없습니다.")
        ...
        # 1. 기존 LineCamera 정보 백업
        original_line_camera = self.ui.LineCamera
        parent = original_line_camera.parentWidget()
        geom = original_line_camera.geometry()

        # 2. 새로운 BoxDrawingView 생성 및 설정
        self.line_camera_view = BoxDrawingView(parent)
        self.line_camera_view.setObjectName("LineCamera")
        self.line_camera_view.setGeometry(geom)
        self.line_camera_view.setScene(self.scene)
        self.line_camera_view.setRenderHint(QPainter.Antialiasing)
        self.line_camera_view.setDragMode(QGraphicsView.ScrollHandDrag)
        self.line_camera_view.show()

        # 3. 기존 LineCamera 숨김 및 참조 교체
        original_line_camera.hide()
        self.ui.LineCamera = self.line_camera_view  # 참조 업데이트
    ...
    def display_pixmap_slice(self, pixmap, start_x, width):
        ...
        ###^^변경vv##
        scene = self.ui.LineCamera.scene()
        if hasattr(self, "image_item"):
            scene.removeItem(self.image_item)

        self.image_item = QGraphicsPixmapItem(sub_pixmap)
        scene.addItem(self.image_item)
        scene.setSceneRect(0, 0, sub_pixmap.width(), height)
        self.ui.LineCamera.fitInView(self.image_item, Qt.IgnoreAspectRatio)
