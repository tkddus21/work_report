...
class TrackBotApp(QMainWindow):
    def __init__(self):
        ...
        self.ui.LineCamera.setViewportUpdateMode(QGraphicsView.FullViewportUpdate)
        ...
        self.scroll_timer.timeout.connect(self.handle_scroll_update)
        self.scroll_timer.start(30)  # 30ms 간격 (약 33fps)
        self.scroll_value_pending = None
        
        # 스크롤바 이벤트 연결 변경
        self.ui.horizontalScrollBar.valueChanged.connect(
            lambda v: setattr(self, "scroll_value_pending", v)
        )
    ...
    def display_pixmap_slice(self, pixmap, start_x, width):
        ...
        self.ui.LineCamera.setUpdatesEnabled(False)
        ...
        self.ui.LineCamera.setUpdatesEnabled(True)
    ...
    def on_scroll_changed(self, value):
        ...
        if (self.scroll_direction == 1 and
            value >= mid_image_start + mid_image_length and  # 현재 이미지의 끝부분이 화면 왼쪽 끝에 도달
            ...
            if next_mid_idx + 1 < len(self.inspection_config):
                ...
                self.transition_buffers(1)

                def on_ready():
                    self.ui.horizontalScrollBar.blockSignals(True)
                    self.animate_scroll_to(int(new_value))
                    self.ui.horizontalScrollBar.blockSignals(False)
                    if self.merged_buffers[1][3]:
                        start_x = new_value - self.merged_buffers[1][0]
        ...
        # 왼쪽으로 이동 (이전 버퍼로 전환)
        elif (self.scroll_direction == -1 and
            value + self.viewer_width <= mid_image_start and  # 현재 이미지의 시작부분이 화면 오른쪽 끝에 도달
            self.last_transition_idx != mid_idx - 1):
    ...
    def animate_scroll_to(self, target_value):
        scrollbar = self.ui.horizontalScrollBar
        anim = QPropertyAnimation(scrollbar, b"value")
        anim.setDuration(300)  # 300ms
    ...
    def handle_scroll_update(self):
        if self.scroll_value_pending is not None:
            value = self.scroll_value_pending
            self.scroll_value_pending = None  # consume
            self.update_display_from_scroll(value)

    def update_display_from_scroll(self, value):
        print(f"[📍 스크롤 위치] {int(value)} px")

        # 스크롤 방향 계산
        self.scroll_direction = 1 if value > self.last_scroll_value else -1 if value < self.last_scroll_value else 0
        self.last_scroll_value = value

        current_time = time.time()
        if current_time - self.last_transition_time < self.transition_cooldown:
            return  # 전환 딜레이 중이면 버퍼만 사용
        ...
        # 전환 조건 (우측)
        if (self.scroll_direction == 1 and
            value >= mid_image_start + mid_image_length and
            self.last_transition_idx != mid_idx + 1):
            
            self.perform_buffer_transition(mid_idx, direction=1, scroll_value=value)
            return

        # 전환 조건 (좌측)
        elif (self.scroll_direction == -1 and
              value + self.viewer_width <= mid_image_start and
              self.last_transition_idx != mid_idx - 1):

            self.perform_buffer_transition(mid_idx, direction=-1, scroll_value=value)
            return

        # 현재 이미지 보여주기 (NumPy 버전)
        if self.image_data[3] and self.image_data[4] is not None:
            start_x = value - self.image_data[0]
            if 0 <= start_x <= self.image_data[4].width() - self.viewer_width:
                self.display_pixmap_slice(self.image_data[4], start_x, self.viewer_width)
    ...
    def perform_buffer_transition(self, mid_idx, direction, scroll_value):
        ...
        if direction == 1:
            ...
            print(f"[➡️ 전환] {mid_idx} → {next_mid_idx}")
            next_start_offset = self.inspection_config[next_mid_idx - 1][4]
            relative_offset = scroll_value - self.inspection_config[mid_idx][4]
            new_value = next_start_offset + relative_offset
            ...
            def on_ready():
                self.ui.horizontalScrollBar.blockSignals(True)
                self.ui.horizontalScrollBar.setValue(int(new_value))  # 애니메이션 제거
                self.ui.horizontalScrollBar.blockSignals(False)
                if self.merged_buffers[1][3]:
                    start_x = new_value - self.merged_buffers[1][0]
                    self.display_pixmap_slice(self.merged_buffers[1][4], start_x, self.viewer_width)
            ...
            print(f"[⬅️ 전환] {mid_idx} → {prev_mid_idx}")
            prev_start_offset = self.inspection_config[prev_mid_idx - 1][4]
            relative_offset = scroll_value - self.inspection_config[mid_idx][4]
            new_value = prev_start_offset + relative_offset
        ...
        self.last_transition_idx = mid_idx + direction
