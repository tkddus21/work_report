...
class TrackBotApp(QMainWindow):
    def __init__(self):
        ...
        # 초기 박스 로드
        self.box_view.load_boxes_absolute("boxes.json", self.image_data[0], self.viewer_width)
    ...
    def on_scroll_changed(self, value):
        ...
        if (self.last_transition_idx is not None and 
            ...
            new_relative_scroll_value = value - self.merged_buffers[1][0]

            self.ui.horizontalScrollBar.blockSignals(True)
            # 스크롤바 최대값 고려하여 설정
            max_scroll = self.ui.horizontalScrollBar.maximum()
            self.ui.horizontalScrollBar.setValue(min(int(value), max_scroll))
        ...
        if (self.scroll_direction == -1 and 
            ...
            self.ui.horizontalScrollBar.setValue(max(int(value), min_scroll))
            self.ui.horizontalScrollBar.blockSignals(False)
    ...
    def handle_box_created(self, x_local):
        """박스 생성 시 호출되어 절대 좌표를 계산하고 파일에 저장"""
        # 마지막에 추가된 박스 아이템 가져오기
        if not self.box_view.box_items:
            return
            
        new_box_item = self.box_view.box_items[-1]
        rect = new_box_item.rect()

        # 현재 버퍼의 시작 오프셋 가져오기
        # current_buffer_offset = self.image_data[0] # 이제 스크롤바 값을 사용

        # 절대 좌표 계산 (화면에 보이는 슬라이스 기준 상대좌표 + 스크롤바의 절대 위치)
        x_absolute = rect.x() + self.ui.horizontalScrollBar.value()
        ...
        # 박스 아이템에 절대 좌표 데이터 저장
        new_box_item.setData(1, box_data)

        # --- 디버그 로그 추가 ---
        print(f"[박스 생성 디버그]")
        print(f" - 로컬 X (뷰어 기준): {rect.x()}")
        print(f" - 버퍼 시작 offset: {self.ui.horizontalScrollBar.value()}")
        print(f" - 절대 X: {x_absolute}")

        found_segment = None
        for i, cfg in enumerate(self.inspection_config):
            start = cfg[4] # total_length is the start offset
            length = cfg[3] # length_pix is the length
            end = start + length
            if start <= x_absolute < end:
                found_segment = i
                print(f" → 세그먼트 {i} 범위 {start}~{end} 에 속함")
                break
        if found_segment is None:
             print(f" → WARNING: 절대 X {x_absolute}가 어떤 세그먼트 범위에도 속하지 않음.")
        # --- 디버그 로그 끝 ---


        # 파일에 박스 데이터 추가
        self.box_view.add_box_absolute("boxes.json", box_data)

    def handle_box_deleted(self, box_data_to_delete):
        """박스 삭제 시 호출되어 파일에서 해당 박스 정보를 삭제"""
        self.box_view.delete_box_absolute("boxes.json", box_data_to_delete)
