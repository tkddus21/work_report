...
class TrackBotApp(QMainWindow):
    def __init__(self):
        ...
        #박스 뷰 save 자동 저장
        self.box_view.boxUpdated.connect(self.save_current_segment_boxes)
        
        #박스생성 신호 
        self.box_view.boxCreated.connect(self.assign_segment_to_box)
    ...
    def on_scroll_changed(self, value):
        ...
        if (value >= mid_image_center_offset and 
            ...
            if next_mid_idx + 1 <= len(self.inspection_config):  # 마지막 체크
                ...
                def on_ready():
                    ...
                    if self.merged_buffers[1][3]:  # valid 체크
                        ...
                        self.load_visible_boxes(new_value, new_value + self.viewer_width)
        ...
        elif (value < mid_image_start and 
              self.scroll_direction == -1 and 
              self.last_transition_idx != mid_idx - 1):
        ...
        if self.image_data[3] and self.image_data[4] is not None:
            ...
            if 0 <= start_x <= self.image_data[4].width() - self.viewer_width:
                ...
                self.load_visible_boxes(value, value + self.viewer_width)
    ...
    ##### 박스 관련 함수 #####
    ####x좌표 offset ####
    def assign_segment_to_box(self, x_pos):
        """x좌표를 기반으로 해당 segment_id를 찾아 박스에 저장"""
        # 현재 버퍼의 정보 가져오기
        current_buffer = self.merged_buffers[self.current_buffer_index]
        buffer_start_offset = current_buffer[0]  # 버퍼의 시작 offset
        buffer_mid_idx = current_buffer[2]      # 버퍼의 중간 이미지 인덱스

        # 박스의 절대 위치 계산 (버퍼 시작점 기준)
        absolute_x = buffer_start_offset + x_pos

        # config 데이터를 순회하며 해당 위치의 세그먼트 찾기
        found_segment = None
        for idx, data in enumerate(self.inspection_config):
            segment_start = data[4]  # total_length (시작 offset)
            segment_length = data[3]  # length_pix
            segment_end = segment_start + segment_length

            if segment_start <= absolute_x < segment_end:
                found_segment = idx
                print(f"[📌 박스 위치 계산]")
                print(f"    - 버퍼 시작 offset: {buffer_start_offset}")
                print(f"    - 박스 상대 위치: {x_pos}")
                print(f"    - 박스 절대 위치: {absolute_x}")
                print(f"    - 세그먼트 {idx} 범위: {segment_start}~{segment_end}")
                
                # 위치가 일치하는 박스에 세그먼트 ID 저장
                for box in self.box_view.box_items:
                    rect = box.rect()
                    if abs(rect.x() - x_pos) < 5:  # 같은 x 위치의 박스
                        box.setData(0, idx)
                        print(f"    - 세그먼트 {idx}에 박스 저장됨")
                break

        return found_segment
    ###확인할 필요있음###
    def save_current_segment_boxes(self):
        """현재 세그먼트의 박스들을 저장"""
        # 박스의 세그먼트 ID를 사용하여 저장
        for box in self.box_view.box_items:
            segment_id = box.data(0)
            if segment_id is not None:
                print(f"[💾 저장] 세그먼트 {segment_id}의 박스 저장")
                self.box_view.save_boxes_dict("boxes.json", segment_id)
                break  # 한 번만 저장하면 됨

    def load_current_segment_boxes(self):
        """현재 세그먼트의 박스들을 로드"""
        # 현재 버퍼의 중간 이미지 인덱스 사용
        segment_id = self.merged_buffers[self.current_buffer_index][2]
        print(f"[📂 로드] 세그먼트 {segment_id}의 박스 로드")
        self.box_view.load_boxes_dict("boxes.json", segment_id)
    ...
    def build_merged_buffer(self, start_idx, buffer_slot, on_ready_callback=None):
        ...
        try:
            ...
            #     # 단, 끝에 도달할 수 있도록 마지막 버퍼는 한 번은 생성하도록 허용
            #     if start_idx + 2 == len(self.inspection_config):   
            #         pass
            #     else:
